    ...
    <div class="container-narrow">
      <article class="article-card">
        <header class="article-header">
          <div class="header-meta">
            <el-tag class="content-type" size="small">{{
              contentTypeText
            }}</el-tag>
          ...
          <div class="creator-row">
            <div class="creator-info" @click="goToCreator">
              <el-avatar :size="48" :src="content.creatorAvatar">{{
                content.creatorName?.charAt(0)
              }}</el-avatar>
              <div class="creator-text">
                <span class="name">{{ content.creatorName }}</span>
                <span class="category">{{
                  content.creatorCategory || "创作者"
                }}</span>
              </div>
            </div>
            <div class="stats">
              <span class="stat-item"
                ><el-icon><View /></el-icon>{{ content.viewCount }}</span
              >
              <span class="stat-item"
                ><el-icon><Star /></el-icon>{{ content.likeCount }}</span
              >
              <span class="stat-item"
                ><el-icon><ChatDotRound /></el-icon>{{ commentTotal }}</span
              >
            </div>
          </div>
        </header>
        ...
        <!-- 视频内容 -->
        <div
          v-if="content.contentType === 2 && content.mediaUrl"
          class="media-section"
        >
          <video
            :src="getFullUrl(content.mediaUrl)"
            controls
            class="video-player"
          ></video>
        </div>
        ...
        <!-- 音频内容 -->
        <div
          v-if="content.contentType === 3 && content.mediaUrl"
          class="media-section"
        >
        ...
        <!-- 图集内容 -->
        <div
          v-if="content.contentType === 4 && content.mediaUrl"
          class="media-section gallery-section"
        >
          <el-image
        ...
        <div class="article-content">
          <div
            v-if="content.isPaid && !content.isPurchased && !isOwner"
            class="paid-notice"
          >
            <div class="paid-icon">
      ...
      <section class="comments-section">
        <div class="section-header">
          <h2>
            评论 <span class="count">({{ commentTotal }})</span>
          </h2>
        </div>
      ...
      <div class="purchase-content">
        <div class="purchase-info">
          ...
          <p class="purchase-desc">
            确定花费 ¥{{ content.price }} 购买此内容吗？
          </p>
          <p class="balance-info">
            当前余额：¥{{ userStore.userInfo?.balance?.toFixed(2) || "0.00" }}
          </p>
        </div>
      </div>
      <template #footer>
        <div class="dialog-footer">
          <el-button size="large" @click="showPurchaseDialog = false"
            >取消</el-button
          >
          <el-button
            type="primary"
    ...
    >
      <div class="reward-content">
        <div class="reward-header">
          <el-avatar :size="72" :src="content.creatorAvatar">{{
            content.creatorName?.charAt(0)
          }}</el-avatar>
        ...
        <div class="reward-amounts">
          <button
            ...
            :class="[
              'amount-btn',
              { active: rewardAmount === amount && !isCustom },
            ]"
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button size="large" @click="showRewardDialog = false"
            >取消</el-button
          >
          <el-button
            type="primary"
...
const contentTypeText = computed(
  () => contentTypeMap[content.value.contentType] || "文章",
);
const isOwner = computed(
  () => userStore.userInfo?.creatorId === content.value.creatorId,
);
const formattedContent = computed(
  () => content.value.content?.replace(/\n/g, "<br>") || "",
);
const displayAmount = computed(() =>
  isCustom.value && customAmount.value
    ? customAmount.value
    : rewardAmount.value,
);
const galleryImages = computed(() =>
  content.value.mediaUrl
    ? content.value.mediaUrl.split("\n").filter((url) => url.trim())
    : [],
);
...
const handleReward = async () => {
  ...
  try {
    await reward({
      creatorId: content.value.creatorId,
      contentId: content.value.id,
      amount: amount,
      message: rewardMessage.value,
    });
